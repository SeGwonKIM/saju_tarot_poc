...
| 과제가 요구한 것 | 이 저장소에서 |
|---|---|
| **문제 정의서** (도메인·문제·가설·사용자·성공기준) | [`문제정의서.md`](문제정의서.md) |
| **PoC 코드** | [`poc/`](poc/) — 파이프라인 7단계 · [`engine/`](engine/) — 만세력 계산 |
| ↳ **노트북으로 읽기** | [`PoC_둘러보기.ipynb`](PoC_둘러보기.ipynb) — **결과가 박혀 있어 클론 없이 읽힙니다** |
| ↳ **모델 선정 근거** | [`모델선정근거.md`](모델선정근거.md) |
| **개선 효과 검증 결과** (기존 방식 비교 · 실패 사례) | [`개선효과검증결과.md`](개선효과검증결과.md) |
| **README** (실행 방법 · 시연 자료) | 이 파일 — 아래 [시연](#시연--실제로-돌린-화면) |
...
`--규칙` 은 LLM 없이 규칙 기반으로만 돌립니다. **API 키 없이 바로 확인**하실 수 있습니다.

> ⚠️ `poc/` 폴더 이름은 **바꾸면 안 됩니다.** 파이썬이 폴더 이름을 모듈 이름으로 쓰기 때문에,
> 괄호나 공백이 들어가면 `python -m poc.pipeline` 이 아예 실행되지 않습니다.
...
| `개선효과검증결과.md` | 실제로 나아졌는가 · 실패 사례 25건 · 기준 변경 대장 |
| `모델선정근거.md` | **왜 이 모델을 골랐는가** · 후보 비교 · 안 쓴 모델과 이유 |
