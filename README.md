...
**검사 건전성** — 변이 시험 **55/55 유효** · 상수 조건 **0곳** · 전칭 양화 미등록 **0곳** · 경계 커버리지 **10/10**
...
[검증 1] 실물 성명장 대조
  PASS  년·월·일주가 실물과 일치   未시 구간 4개 시각 모두 丙午 丙申 丙辰
...
  PASS  경도 1도 = 4분 규칙을 지킴   실제 16분 / 이론 15.7분
  PASS  경도 차이로 시주가 실제로 갈리는 시각이 존재   01:24 — 서울 戊子 / 울릉도 己丑

[경계 커버리지] 어떤 경계 입력을 실제로 밟았는가
  밟음  윤달 출생          밟음  없는 윤달 거부      밟음  자시 경계(23~01시)
  밟음  입춘 전후(2월 초)    밟음  절기 당일          밟음  서머타임 연도
  밟음  127.5° 시기       밟음  경도 극단(울릉도)     밟음  시간 미상(3주)
  밟음  서비스 범위 밖 거부(1940 이전)
  경계 커버리지 10/10
...
| `docs/검토대응.md` | 외부 검토 32건에 무엇을 했는가 |
